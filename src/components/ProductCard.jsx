...
export function ProductCard({ product, view = 'grid' }) {
  ...
  const stock = getStockStatus(product.id, product.inStock);

  // Flash animation when stock changes in real-time
  const [flashing, setFlashing] = useState(false);
  const prevInStock = useRef(product.inStock);

  useEffect(() => {
    if (stock.isRealTime && stock.inStock !== prevInStock.current) {
      setFlashing(true);
      prevInStock.current = stock.inStock;
      const timer = setTimeout(() => setFlashing(false), 800);
      return () => clearTimeout(timer);
    }
  }, [stock.inStock, stock.isRealTime]);

  // Merge real-time stock into product for child components
  const liveProduct = { ...product, inStock: stock.inStock };
    ...
    return (
      <article
        className={`group flex gap-4 rounded-lg border border-[--color-line] p-4
          ...
          ${flashing ? 'animate-inventory-flash' : ''}`}
      >
        <Link to={`/product/${product.id}`} className="shrink-0 group-hover:opacity-90">
          <ProductImage product={liveProduct} className="w-24 h-24" />
        </Link>
    ...
    >
      <div className="relative">
        <Link to={`/product/${product.id}`} className="block group-hover:opacity-90">
          <ProductImage product={liveProduct} className="aspect-square w-full" />
        </Link>
